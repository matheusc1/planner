...
test('change location and date', async ({ page }) => {
  ...
  await expect(page.getByText(`7 de ${nextMonth} até 24 de ${nextMonth}`)).toBeVisible()
})
...
test('create new activity', async ({ page }) => {
  ...
  await page.getByPlaceholder('Data e horário da atividade').fill(format(nextMonthDate, 'yyyy-MM-10\'T\'11:00', { locale: ptBR }))
  await page.getByRole('button', { name: 'Salvar atividade' }).click()

  await page.getByRole('button', { name: 'Cadastrar atividade' }).click()
  await page.getByPlaceholder('Qual a atividade?').fill('Jantar')
  await page.getByPlaceholder('Data e horário da atividade').fill(format(nextMonthDate, 'yyyy-MM-10\'T\'20:00', { locale: ptBR }))
  await page.getByRole('button', { name: 'Salvar atividade' }).click()
...
test('register new link', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'https://www.tudosobrepequim.' })).toBeVisible()
})
