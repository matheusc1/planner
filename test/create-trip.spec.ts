...
async function createTrip(page: Page, location: string, startDay: string, endDay: string) {
...
async function updateTripDates( page: Page, location: string, previousStartDay: string, startDay: string, endDay: string) {
  await page.getByPlaceholder('Para onde você vai?').fill(location)
  await page.locator('[data-testid="date-button"]').click()
  await page.getByLabel('Go to next month').click()
...
test('display the correct location and date', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="date-button"]')).toHaveText(formatedDate)
  ...
  await expect(page.locator('[data-testid="date-button"]')).toHaveText(`20 de ${nextMonth} até 28 de ${nextMonth}`)
})
...
test('confirm trip creation', async ({ page }) => {
  ...
  .toHaveText(`Para concluir a criação da viagem para Ulan Bator, Mongólia nas datas de ${formatedDate} preencha seus dados abaixo:`)
